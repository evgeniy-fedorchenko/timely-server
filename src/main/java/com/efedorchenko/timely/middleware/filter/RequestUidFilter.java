package com.efedorchenko.timely.middleware.filter;
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request,
        ...
        try {
            ...
            if (log.isDebugEnabled() && this.isAsyncStarted(request)) {
                log.trace("Async processing: [{} {}], ip: [{}]",
                        request.getMethod(), request.getRequestURI(), request.getRemoteAddr());
            }

            if (log.isDebugEnabled() && !this.isAsyncStarted(request)) {
                log.trace("Finish processing: [{} {}], ip: [{}]",
                        request.getMethod(), request.getRequestURI(), request.getRemoteAddr());
            }
        } catch (Throwable t) {
            log.error("Error processing: [{} {}], ip: [{}]",
                    request.getMethod(), request.getRequestURI(), request.getRemoteAddr(), t);
        } finally {
            MDC.clear();
        }
    }
